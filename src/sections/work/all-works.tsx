		...
		<div className="bg-bgprimary w-full px-2 md:px-12 lg:px-48 padding-y lg:py-36 flex flex-col items-center justify-center gap-20">
			...
			>
				{initialProjects.map((project, index) => (
					<ItemCard key={index} title={project.name} img={project.img} link={project.link} />
				))}
			</motion.div>
        ...
        {visibleProjects > VIEW_COUNT && (
          ...
          >
            {additionalProjects.map((project, index) => (
              <ItemCard key={index + VIEW_COUNT} title={project.name} img={project.img} link={project.link} />
            ))}
          </motion.div>
        )}
      </AnimatePresence>
